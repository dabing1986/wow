--版本控制：2.0.6 新增MailLog（Recieved）功能
...
function Addon:AnnounceTrade()
	...
	if (TradeLog[#TradeLog].Result == "error" or TradeLog[#TradeLog].Result == "cancelled") and TradeLog[#TradeLog].TargetName and TradeLog[#TradeLog].Reason then
		local msg = string.format(L["SPELLWHISPER_TEXT_TRADE_ERROR"], TradeLog[#TradeLog].TargetName, TradeLog[#TradeLog].Reason)
	...
	elseif TradeLog[#TradeLog].Result == "completed" then
		...
		local msg = string.format(L["SPELLWHISPER_TEXT_TRADE_SUCCEED"], TradeLog[#TradeLog].TargetName)
		if ReceiveMoneyString ~= "" then
			msg = msg .. string.format(L["SPELLWHISPER_TEXT_TRADE_MONEY_RECEIVE"], ReceiveMoneyString)
		end
		if GiveMoneyString ~= "" then
			msg = msg .. string.format(L["SPELLWHISPER_TEXT_TRADE_MONEY_GIVE"], GiveMoneyString)
		end
		if ReceiveItemNum > 0 then
			msg = msg .. Addon:FormatMessage(L["SPELLWHISPER_TEXT_TRADE_ITEMS_RECEIVE"],
				{
		...
		if GiveItemNum > 0 then
			msg = msg ..  Addon:FormatMessage(L["SPELLWHISPER_TEXT_TRADE_ITEMS_GIVE"],
				{
		...
		if TradeLog[#TradeLog].GiveItems[7] then
			msg = msg .. string.format(L["SPELLWHISPER_TEXT_TRADE_ENCHANTMENT"], TradeLog[#TradeLog].GiveItems[7].ItemLink, TradeLog[#TradeLog].GiveItems[7].Enchantment)
		end
		if TradeLog[#TradeLog].ReceiveItems[7] then
			msg = msg .. string.format(L["SPELLWHISPER_TEXT_TRADE_ENCHANTMENT"], TradeLog[#TradeLog].ReceiveItems[7].ItemLink, TradeLog[#TradeLog].ReceiveItems[7].Enchantment)
		end
		...
		if IsInRaid() then
			for i = 1, GetNumGroupMembers() do
				...
				if UnitGUID(u .. "target") == TargetGUID and not UnitIsUnit(u, "player") then
					if not Addon:CompareOutputCache(msg, (UnitName(u))) and msg ~= L["NONE"] then
						SendChatMessage(msg, AnnounceChannel, nil, (UnitName(u)))
					end
				end
			end
		elseif IsInGroup() then
...
-- 输出交易记录信息
function Addon:PrintTradeLog(ListMode)
	...
	if #TradeLog > 0 then
		for i = #TradeLog, 1, -1 do
			if not TradeLog[i].Date or not TradeLog[i].Time or not TradeLog[i].TargetName or not TradeLog[i].Location or not TradeLog[i].Result then
				t_remove(TradeLog, i)
			end
		end
	end
	-- 没有记录
	if #TradeLog == 0 then
		Output.export:SetText(L["<|cFFBA55D3TL|r>No Trade Log!"].."\n")
		return
	end
	-- 寻找起始点
	local StartPoint, Count = 1, 0
	for i = #TradeLog, 1, -1 do
		if ListMode == "ALL" or ListMode == "TRADE" and TradeLog[i].Result == "completed" or ListMode == "MAIL" and (TradeLog[i].Result == "sent" or TradeLog[i].Result == "received") then
			Count = Count + 1
		end
		if Count > 255 then
			StartPoint = i
	...
	for i = StartPoint, #TradeLog do	-- 限制输出Log数量，避免资源耗尽
		...
		elseif TradeLog[i].Result == "sent" and (ListMode == "ALL" or ListMode == "MAIL") then
			msg = msg .. string.format(L["[|cFFFFFF00%s %s|r]\n    |cFF00FF00%s|r sent a mail to |cFF00FF00%s|r"], TradeLog[i].Date, TradeLog[i].Time, TradeLog[i].PlayerName, TradeLog[i].TargetName) .. "\n"
		elseif TradeLog[i].Result == "received" and (ListMode == "ALL" or ListMode == "MAIL") then
		...
		if (TradeLog[i].ReceiveMoney > 0 or TradeLog[i].GiveMoney > 0) and ((TradeLog[i].Result == "completed" and (ListMode == "ALL" or ListMode == "TRADE")) or (TradeLog[i].Result == "sent" or TradeLog[i].Result == "received") and (ListMode == "ALL" or ListMode == "MAIL")) then
		...
		elseif TradeLog[i].Result == "received" and (ListMode == "ALL" or ListMode == "MAIL") and (next(TradeLog[i].ReceiveItems)) then
			msg = msg .. "    " .. L["|cFFDAA520Receive Item(s)|r: "] .. "\n"
			local j = 1
			for k, v in pairs(TradeLog[i].ReceiveItems) do
				msg = msg .. "    [" .. j .. "] " .. v.ItemLink .. " (" .. v.Number .. ")" .. "\n"
		...
		elseif TradeLog[i].Result == "sent" and (ListMode == "ALL" or ListMode == "MAIL") and (next(TradeLog[i].GiveItems)) then
			msg = msg .. "    " .. L["|cFFFF4500Give Item(s)|r: "] .. "\n"
			local j = 1
			for k, v in pairs(TradeLog[i].GiveItems) do
				msg = msg .. "    [" .. j .. "] " .. v.ItemLink .. " (" .. v.Number .. ")" .. "\n"
...
function Frame:ADDON_LOADED(Name)
	...
	if not SpellWhisperDB.TradeLog then
		SpellWhisperDB.TradeLog = {}
	end
	-- 用SpellWhisperDB更新config和SpellList表，以及TradeLog表
	Addon:UpdateTable(Config, SpellWhisperDB.Config)
	Addon:UpdateTable(RunTime, SpellWhisperDB.SpellList)
	Addon:UpdateTable(TradeLog, SpellWhisperDB.TradeLog)
	-- 初始化Output
...
function Frame:PLAYER_LOGOUT()
	...
	Addon:UpdateTable(SpellWhisperDB.TradeLog, TradeLog)
end
	...
	if t == "group" then
		if Config.StartFollow ~= "" and arg[1] == Config.StartFollow then --跟随
			if not CheckInteractDistance(u, 4) then
				SendChatMessage(L["SPELLWHISPER_TEXT_FOLLOWFAILED_OUTOFRANGE"], "whisper", nil, (UnitName(u)))
			else
				FollowUnit(u)
				Addon.Followed.UnitIndex = u
				Addon.Followed.GUID = arg[12]
				Addon.Followed.StartTime = GetTime()
				SendChatMessage(L["SPELLWHISPER_TEXT_FOLLOWREPLY"], "whisper", nil, (UnitName(u)))
			end
		elseif Config.StopFollow ~= "" and arg[1] == Config.StopFollow then --停止跟随
			FollowUnit("player")
			Addon.Followed.UnitIndex = "player"
			Addon.Followed.GUID = ""
			SendChatMessage(L["SPELLWHISPER_TEXT_STOPFOLLOWREPLY"], "whisper", nil, (UnitName(u)))
		elseif Config.CombatFollowSwitchKey ~= "" and arg[1] == Config.CombatFollowSwitchKey then
			if Config.CombatFollow then
				...
				SendChatMessage(L["SPELLWHISPER_TEXT_CHANGEFOLLOWMODEOFF"], "whisper", nil, (UnitName(u)))
			else
				Config.CombatFollow = true
				SendChatMessage(L["SPELLWHISPER_TEXT_CHANGEFOLLOWMODEON"], "whisper", nil, (UnitName(u)))
			end
...
function Frame:COMBAT_LOG_EVENT_UNFILTERED(...)
	...
	if arg[2] == "SPELL_CAST_FAILED" and arg[4] == PlayerGUID and InInstance then --治疗法术失败提示
		...
		if FailedReason then
			...
			elseif
				SpellName and (UnitName("targettarget")) and UnitIsPlayer("targettarget") and not UnitIsFriend("player", "target") and
					UnitIsFriend("player", "targettarget")
			 then
				CurrentTarget = "targettarget"
			end
			if CurrentTarget then
				Addon:SendHealingMessage(SpellName, (UnitName(CurrentTarget)), FailedReason)
			end
		end
	...
	elseif arg[2] == "SPELL_CAST_START" and arg[4] == PlayerGUID then
		...
		if SpellName then
			...
			if CurrentTarget then
				...
				Addon:SendAnnounceMessage("group", "start", TypeHelpHarm, SpellName, (UnitName(CurrentTarget)), RaidTargeIcon)
				if CurrentTarget and not IsIgnoreSpell then
					if TypeHelpHarm == "help" and UnitIsPlayer(CurrentTarget) then
						-- Addon:SendAnnounceMessage("whisper", "start", TypeHelpHarm, SpellName, (UnitName(CurrentTarget)), RaidTargeIcon)
	...
	elseif arg[2] == "SPELL_CAST_SUCCESS" and arg[4] == PlayerGUID then --增益通告:arg[4]施法者GUID，arg[9]目标名字，arg[8]目标GUID(不再使用GUID)
		...
		if SpellName then
			...
			if arg[9] then
				...
				NowGUID = TargetGUID or PlayerGUID
			end
			local _, u = Addon:GetTargetUnit(NowGUID)
			if UnitIsPlayer(u) then
				local RaidTargeIcon = GetRaidTargetIndex(u) or 0
				Addon:SendAnnounceMessage("group", "done", TypeHelpHarm, SpellName, (UnitName(u)), RaidTargeIcon)
				Addon:SendAnnounceMessage("whisper", "done", TypeHelpHarm, SpellName, (UnitName(u)), RaidTargeIcon)
			end
		end
	elseif
	...
	if Addon.Followed.UnitIndex ~= "player" then
		if CheckInteractDistance(Addon.Followed.UnitIndex, 4) then
			if math.floor(GetTime() * 10) / 10 - Addon.Followed.StartTime > 0.1 then
				SendChatMessage(L["SPELLWHISPER_TEXT_STOPFOLLOW_MANUALLY"], "whisper", nil, (UnitName(Addon.Followed.UnitIndex)))
				Addon.Followed.UnitIndex = "player"
				Addon.Followed.GUID = ""
			end
		end
...
function Frame:TRADE_SHOW()
	...
	if Config.PreventTrade then
		local RequestName = (UnitName("npc"))
		local TargetCanTradeMe = false
		if RequestName then
			if (UnitName("target")) == RequestName then
				TargetCanTradeMe = true
			end
			if UnitGUID("npc") and not TargetCanTradeMe then
				if IsFriend(UnitGUID("npc")) then
					TargetCanTradeMe = true
				end
			end
			if IsInGuild() and not TargetCanTradeMe then
				if IsGuildMember(RequestName) then
					TargetCanTradeMe = true
				end
			end
...
do -- Hook TakeIndexItem，获取邮件取出的邮件及物品信息
	local function GetItemFromMail(MailIndex, ItemIndex)
		...
		if not TradeLog[#TradeLog] or (TradeLog[#TradeLog].TargetName and TradeLog[#TradeLog].TargetName ~= Sender) then
