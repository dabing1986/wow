...
Elixir of the Mongoose, Mageblood Potion, Elixir of Fortitude, Elixir of Superior Defense, Major Troll's Blood Potion, Gift of Arthas, Juju Power, Elixir of Giants, Juju Might, Winterfall Firewater, Greater Arcane Elixir, Elixir of Shadow Power, Elixir of Greater Firepower, Elixir of Frost Power, Juju Ember, Juju Chill, Crystal Ward, Crystal Spire, Grilled Squid, Smoked Desert Dumplings, Nightfin Soup, Runn Tum Tuber Surprise, Dirge's Kickin' Chimaerok Chops, Blessed Sunfruit Juice, Blessed Sunfruit, Kreeg's Stout Beatdown, Gordok Green Grog, Rumsey Rum Black Label, Greater Arcane Protection Potion, Greater Fire Protection Potion, Greater Frost Protection Potion, Greater Nature Protection Potion, Greater Shadow Protection Potion, Noggenfogger Elixir, Savory Deviate Delight
...
1.35
- optional: Remove Aspect of the Cheetah/Aspect of the Pack direct before a combat (default on)
...
- Add: Dampen Magic
- Optional: Automatic Cancelling of "Power Word: Shield" and "Blessing of Salvation / Greater Blessing of Salvation"

1.32
- optional: Add current Target in List.
- new downgrade system. When a spell is rejected with a "player is to low"-errormessage, the next time buff'o'mat try to cast a lower version of the spell.

1.31
- List sorted by name
- correct maximum macro limit
- Blessings - it was possible, that to many manual single-Target will trigger a group buff.
...
function BOM.Init()
	...
	if not buffOmatDB then buffOmatDB = {} end
	if not buffOmatDB.Minimap then buffOmatDB.Minimap = {} end
	if not buffOmatDB.SpellGreatherEqualThan then buffOmatDB.SpellGreatherEqualThan = {} end
	if not buffOmatDB.CustomLocales then buffOmatDB.CustomLocales = {} end
	if not buffOmatDB.CustomSpells then buffOmatDB.CustomSpells = {} end
	if not buffOmatDB.CustomCancelBuff then buffOmatDB.CustomCancelBuff = {} end
	
	if not buffOmatDBChar then buffOmatDBChar = {} end
	--if not buffOmatDBChar.Duration then buffOmatDBChar.Duration={} end
	if buffOmatDBChar.Duration then
		buffOmatDB.Duration=buffOmatDBChar.Duration
		buffOmatDBChar.Duration=nil
	elseif not buffOmatDB.Duration then
		buffOmatDB.Duration={}
	end
	
	if not buffOmatDBChar[BOM.ProfileNames[1]] then
		buffOmatDBChar[BOM.ProfileNames[1]]={
			["CancelBuff"] = buffOmatDBChar.CancelBuff,
			["Spell"] = buffOmatDBChar.Spell,
			["LastAura"]= buffOmatDBChar.LastAura,
			["LastSeal"]= buffOmatDBChar.LastSeal,
		}
		buffOmatDBChar.CancelBuff=nil
		buffOmatDBChar.Spell=nil
		buffOmatDBChar.LastAura=nil
		buffOmatDBChar.LastSeal=nil
	end
	
	for i,profil in ipairs(BOM.ProfileNames) do
		if not buffOmatDBChar[profil] then buffOmatDBChar[profil]={} end
	end
	
	
	BOM.DB = buffOmatDB
	BOM.DBChar = buffOmatDBChar
	BOM.CurrentProfile = buffOmatDBChar[BOM.ProfileNames[1]]
	...
	BOM.Tool.EnableSize(BuffOmat_MainWindow,8,nil,function() BOM.SaveWindowPosition()	end)
	...
	BOM.Tool.SlashCommand({"/bom","/buffomat"},{
		...
		{"update",L["SlashUpdate"],function() BOM.ForceUpdate=true BOM.UpdateScan() end},
		{"updatespellstab","",BOM.UpdateSpellsTab},
		{"close",L["SlashClose"],BOM.HideWindow},
		{"reset",L["SlashReset"],BOM.ResetWindow},
		{"_checkforerror","",function() if not InCombatLockdown() then BOM.CheckForError=true end end},
	...
	_G["BINDING_HEADER_BUFFOMATHEADER"]="Buff'o'mat"
...
function BOM.UpdateSpellsTab()
	if BOM.Spells==nil then return end
	if InCombatLockdown() then return end
