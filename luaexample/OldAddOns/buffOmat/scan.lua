...
function BOM.GetSpells()
	...
	if not SpellsIncluded then
		SpellsIncluded=true
		for x,entry in ipairs(buffOmatDB.CustomSpells) do
			tinsert(BOM.SpellList, BOM.Tool.CopyTable(entry))
		end
		for x,entry in ipairs(buffOmatDB.CustomCancelBuff) do
...
function BOM.GetPartyMembers() -- and buffs
...
function BOM.TimeCheck(ti,duration)
	if ti==nil or duration==nil or ti==0 or duration==0 then return true end
	local dif
	if duration<=60 then
		dif=BOM.DB.Time60
...
local function OutputDisplay()
	table.sort(display,function (a,b) return a[2] > b[2] or (a[2]==b[2] and a[1]>b[1]) end)
	table.sort(displayInfo,function (a,b) return a[1] > b[1] end)
...
function BOM.BattleCancelBuffs()
	if BOM.Spells==nil or BOM.CurrentProfile==nil then return end
	for i,spell in ipairs(BOM.CancelBuffs) do
